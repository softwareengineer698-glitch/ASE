...
class MockDataService {
  ...
  static List<ForecastData> getMockForecastData() {
    ...
    for (int i = 0; i < 30; i++) {
      ...
      for (String category in categories) {
        ...
        final double baseDemand = _getBaseDemandForCategory(category);
        final double seasonalFactor = _getSeasonalFactor(date, category);
        final double randomFactor = 0.8 + (i % 5) * 0.1; // Simple variation
        
        final double demand = baseDemand * seasonalFactor * randomFactor;
        final double confidence = 0.7 + (i % 3) * 0.1; // Confidence decreases over time
  ...
  static double _getSeasonalFactor(DateTime date, String category) {
    ...
    final double seasonalCycle = (dayOfYear % 30) / 30.0; // Simple 30-day cycle
  ...
  static ForecastSummary getForecastSummary(String category) {
    ...
    final double totalDemand = forecasts.fold(0, (sum, f) => sum + f.demand);
    final double averageDemand = totalDemand / forecasts.length;
    
    final ForecastData peakForecast = forecasts.reduce((a, b) => a.demand > b.demand ? a : b);
