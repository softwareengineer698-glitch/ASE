...
class DemoService {
  ...
  Future<void> _loadDefaultScenario() async {
    ...
    await _profileService.createProfile(
      ...
      bio: 'Passionate about reducing food waste and helping the community through FoodBridge.',
    );
    ...
    for (final item in demoItems) {
      await _surplusService.addSurplusItem(
        ...
        expiryDate: DateTime.now().add(Duration(days: item['expiryDays'] as int)),
    ...
    await _addDemoNotifications([
      {
        ...
        'message': 'Green Valley Farm has donated 45kg of fresh organic vegetables. Perfect for your community kitchen!',
        'hoursAgo': 2,
      },
      {
        'type': NotificationType.surplusAccepted,
        'title': '✅ Your Bread Donation Accepted!',
        'message': 'Community Food Bank has accepted your artisan bread donation. They\'ll pick it up within 2 hours.',
        'hoursAgo': 4,
      },
      {
        'type': NotificationType.general,
        'title': '🎉 Welcome to FoodBridge!',
        'message': 'Thank you for joining our mission to reduce food waste and feed communities. Every donation makes a difference!',
        'hoursAgo': 24,
      },
    ]);
  }
  ...
  Future<void> _loadHighActivityScenario() async {
    await _profileService.createProfile(
      ...
      bio: 'Managing a high-volume restaurant and committed to zero food waste.',
    );

    // Generate many surplus items
    final foodTypes = [
      'Fresh Vegetables', 'Bread & Bakery', 'Dairy Products', 'Prepared Meals',
      'Fruits', 'Meat & Poultry', 'Seafood', 'Grains & Rice', 'Canned Goods',
      'Frozen Items', 'Beverages', 'Desserts', 'Salads', 'Soups'
    ];

    final donors = [
      'Metro Grill', 'City Supermarket', 'Fresh Market', 'Corner Deli',
      'Pizza Palace', 'Healthy Cafe', 'Family Restaurant', 'Food Truck Co.'
    ];
  ...
  Future<void> _loadDonorFocusedScenario() async {
    await _profileService.createProfile(
      ...
      bio: 'Award-winning chef and restaurant owner dedicated to sustainable practices and community support.',
    );

    // Focus on donor's perspective with their donations
    final donorItems = [
      'Organic Quinoa Salad', 'Vegan Protein Bowls', 'Fresh Herb Collection',
      'Artisan Sourdough', 'Seasonal Vegetable Medley', 'Plant-Based Desserts'
    ];
    ...
    await _addDemoNotifications([
      {
        ...
        'message': 'Helping Hands NGO has accepted your organic quinoa salad donation. Impact: 25 meals for families in need.',
        'hoursAgo': 1,
      },
      {
        'type': NotificationType.surplusCollected,
        'title': '✅ Donation Successfully Collected!',
        'message': 'Your vegan protein bowls have been collected and are now feeding 20 community members. Thank you!',
        'hoursAgo': 6,
      },
    ]);
  }
  ...
  Future<void> _loadNGOFocusedScenario() async {
    await _profileService.createProfile(
      ...
      bio: 'Coordinating food rescue operations and community meal programs. Every rescued meal matters.',
    );

    // Many available items for NGO to choose from
    final availableItems = [
      'Restaurant Quality Meals', 'Fresh Produce Mix', 'Bakery Surplus',
      'Dairy & Eggs', 'Frozen Prepared Foods', 'Canned Goods Collection',
      'Fresh Meat & Poultry', 'Seasonal Fruits', 'Vegetarian Options'
    ];

    final donors = [
      'Premium Restaurant', 'Gourmet Market', 'Family Diner', 'Cafe Corner',
      'Food Mart', 'Organic Store', 'Catering Company', 'Hotel Kitchen'
    ];
    ...
    await _addDemoNotifications([
      {
        ...
        'message': 'High-end restaurant has 30 gourmet meals available. Perfect for your evening service!',
        'hoursAgo': 0.5,
      },
      {
        'type': NotificationType.surplusReported,
        'title': '🥖 Fresh Bakery Items Ready!',
        'message': 'Local bakery has surplus bread, pastries, and desserts. Great for breakfast program!',
        'hoursAgo': 2,
      },
    ]);
  }
  ...
  Future<void> _loadEmergencyScenario() async {
    ...
    final urgentItems = [
      'Emergency Food Kits', 'Ready-to-Eat Meals', 'Baby Formula & Food',
      'Medical Nutrition Drinks', 'Non-Perishable Essentials', 'Water & Beverages'
    ];
    ...
    for (final item in urgentItems) {
      await _surplusService.addSurplusItem(
        ...
        expiryDate: DateTime.now().add(Duration(hours: Random().nextInt(24) + 6)),
    ...
    await _addDemoNotifications([
      {
        ...
        'message': 'Large quantity of emergency food kits available for immediate distribution. Contact donor ASAP!',
        'hoursAgo': 0.1,
      ...
      },
      {
        ...
        'message': 'Emergency food distribution network is now active. Priority given to urgent requests.',
        'hoursAgo': 1,
  ...
  Future<void> _loadSuccessStoryScenario() async {
    await _profileService.createProfile(
      ...
      bio: 'Proud FoodBridge partner with 500+ successful donations and zero food waste achieved!',
    );

    // Mix of completed and ongoing donations
    final successItems = [
      {'food': 'Thanksgiving Feast Portions', 'status': SurplusStatus.collected},
    ...
    await _addDemoNotifications([
      {
        ...
        'message': 'Congratulations! Your 500th donation has been successfully collected. You\'ve helped feed 2,500+ people!',
        'hoursAgo': 2,
      },
      {
        'type': NotificationType.general,
        'title': '🏆 Community Impact Award',
        'message': 'You\'ve been nominated for the Community Impact Award for your outstanding contribution to reducing food waste!',
        'hoursAgo': 24,
      },
    ]);
  }

  Future<void> _addDemoNotifications(List<Map<String, dynamic>> notifications) async {
    for (final notif in notifications) {
      await _notificationService.addNotification(
        AppNotification(
          ...
          priority: notif['priority'] as NotificationPriority? ?? NotificationPriority.medium,
  ...
  Future<void> simulateLiveActivity() async {
    ...
    final foodTypes = ['Fresh Salads', 'Soup of the Day', 'Dinner Specials', 'Bakery Items'];
